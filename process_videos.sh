...
#     mask_video="${input_video/_normals.mp4/_relight_1.mp4}"
...
# Iterate over each file in the directory that ends with _normals.mp4
for input_video in "$VIDEO_DIR"/*_albedo.mp4; do
    # Generate the corresponding mask video path by replacing "normals" with "albedo"
    mask_video="${input_video/_albedo.mp4/_relight_1.mp4}"
...
# Iterate over each file in the directory that ends with _normals.mp4
for input_video in "$VIDEO_DIR"/*_roughness.mp4; do
    # Generate the corresponding mask video path by replacing "normals" with "roughness"
    mask_video="${input_video/_roughness.mp4/_relight_1.mp4}"
